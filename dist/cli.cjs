...
var DEFAULT_CONFIG_FILE = ".deloitte-postman.yml";
var DEFAULT_DELOITTE_CONFIG = `schemaVersion: 1
...
function parseDeloitteConfig(value) {
  let parsed;
  try {
    parsed = (0, import_yaml.parse)(value);
  } catch (error) {
  ...
  const invalidMemberPolicy2 = optionalString(scanner.invalidMemberPolicy, "scanner.invalidMemberPolicy");
  if (invalidMemberPolicy2 && invalidMemberPolicy2 !== "continue" && invalidMemberPolicy2 !== "fail") {
  ...
  const config = {
    ...
    scanner: {
      ...
      ...invalidMemberPolicy2 ? { invalidMemberPolicy: invalidMemberPolicy2 } : {},
...
async function loadDeloitteConfig(path, options = {}) {
  ...
  return { path: resolvedPath, config: parseDeloitteConfig(await (0, import_promises.readFile)(resolvedPath, "utf8")) };
}
function pathFromConfig(configPath, value) {
  if (!value?.trim()) return void 0;
  return (0, import_node_path.resolve)(configPath ? (0, import_node_path.dirname)(configPath) : process.cwd(), value);
}
async function loadIdentityMap(path) {
  if (!path) return {};
  const text = await (0, import_promises.readFile)(path, "utf8");
  if ((0, import_node_path.extname)(path).toLowerCase() === ".csv") {
    const result2 = {};
    for (const [index, line] of text.split(/\r?\n/u).entries()) {
      ...
      result2[login.toLowerCase()] = email.toLowerCase();
    }
    return result2;
  }
  let parsed;
...
function firstString(...values) {
  for (const value of values) {
    const normalized = optionalString2(value);
...
function permissionCandidates(member) {
  ...
  for (const value of [
    ...
    const permission = optionalString2(value)?.toLowerCase();
...
function parseRoleMap(value) {
  ...
  for (const [key, target] of Object.entries(parsed)) {
    ...
    const normalizedTarget = optionalString2(target);
...
function unwrapMembers(value) {
  ...
  if (value && typeof value === "object") {
    const record2 = value;
    if (Array.isArray(record2.members)) return record2.members;
    if (Array.isArray(record2.collaborators)) return record2.collaborators;
  }
  throw new Error("Members input must be an array or an object with a members/collaborators array.");
}
function parseMembersReport(value, roleMap2, options = {}) {
  let parsed;
  try {
    parsed = JSON.parse(value);
  } catch (error) {
  ...
  const blockedEmails = /* @__PURE__ */ new Set();
  const unresolved = [];
  const excluded = [];
  ...
  for (const [index, raw] of rawMembers.entries()) {
    ...
    const member = raw;
    const githubLogin = optionalString2(member.login);
    const suppliedEmail = firstString(member.email)?.toLowerCase();
    const mappedEmail = githubLogin ? identityMap.get(githubLogin.toLowerCase()) : void 0;
    const email = suppliedEmail ?? mappedEmail;
    const identifier = email ?? (githubLogin ? `@${githubLogin}` : `member[${index}]`);
    const bot = optionalString2(member.type)?.toLowerCase() === "bot" || githubLogin?.toLowerCase().endsWith("[bot]");
    ...
    const permission = candidates.find((candidate) => roleMap2[candidate]);
    const fallbackRole = optionalString2(options.defaultWorkspaceRole);
    const workspaceRole = explicitRole ?? (permission ? roleMap2[permission] : void 0) ?? fallbackRole;
    if (!workspaceRole) {
      unresolved.push({
        index,
        identifier: email,
...
function renderText(result, workspaceUrl, sourceRepository, gettingStartedUrl, helpUrl) {
...
function renderHtml(result, workspaceUrl, sourceRepository, gettingStartedUrl, helpUrl) {
  const copy = statusCopy(result);
  ...
  return [
    ...
    ...gettingStartedUrl ? [`<p><a href="${escapeHtml(gettingStartedUrl)}">Open the Deloitte Postman getting-started guide</a></p>`] : [],
...
async function writeNotificationEnvelope(path, envelope) {
  const outputPath = (0, import_node_path2.resolve)(path);
  await (0, import_promises2.mkdir)((0, import_node_path2.dirname)(outputPath), { recursive: true });
  await (0, import_promises2.writeFile)(outputPath, `${JSON.stringify(envelope, null, 2)}
`, { mode: 384 });
  return outputPath;
}
...
async function discoverMembersFile(searchRoot = process.cwd()) {
  const root = (0, import_node_path3.resolve)(searchRoot);
  const matches = [];
  try {
  ...
  if (matches.length > 1) {
    const candidates = matches.map((path) => (0, import_node_path3.relative)(root, path) || path).join(", ");
    throw new Error(`Multiple scanner outputs were found under ${root}: ${candidates}. Set members-file explicitly.`);
  }
  return matches[0];
}
async function resolveMembersInput(membersJson, membersFile, roleMapJson, scannerSearchRoot, defaultWorkspaceRole, memberOptions = {}) {
  ...
  const source = inline ?? await (0, import_promises3.readFile)(path, "utf8");
  ...
  return {
    ...
    source: inline ? "inline JSON" : (0, import_node_path3.resolve)(path),
    discovered
  };
}
...
function buildValidationReport(members, source, resolution = {}) {
...
async function packageRoot() {
  const entrypoint = await (0, import_promises4.realpath)(process.argv[1] ?? "dist/cli.cjs");
  return (0, import_node_path4.resolve)((0, import_node_path4.dirname)(entrypoint), "..");
}
async function installStarterKit(target, upgrade = false) {
  const sourceRoot = await packageRoot();
  const consumerRoot = (0, import_node_path4.resolve)(target);
  if (!await exists(consumerRoot)) throw new Error(`Target repository does not exist: ${consumerRoot}`);
  const actionRoot = (0, import_node_path4.resolve)(consumerRoot, ".github/actions/deloitte-postman-workspace-access");
  const destinations = {
    actionRoot,
    workflow: (0, import_node_path4.resolve)(consumerRoot, ".github/workflows/deloitte-postman-workspace-access.yml"),
    pendingWorkflow: (0, import_node_path4.resolve)(consumerRoot, ".github/workflows/deloitte-postman-pending-reconcile.yml"),
    doctor: (0, import_node_path4.resolve)(consumerRoot, "scripts/deloitte-postman-doctor.sh"),
    runbook: (0, import_node_path4.resolve)(consumerRoot, "docs/deloitte-postman-workspace-access.md"),
    prerequisites: (0, import_node_path4.resolve)(consumerRoot, "docs/deloitte-postman-prerequisites.md"),
    sandbox: (0, import_node_path4.resolve)(consumerRoot, "docs/deloitte-postman-sandbox-smoke.md"),
    notifications: (0, import_node_path4.resolve)(consumerRoot, "docs/deloitte-postman-notifications.md"),
    emailTemplate: (0, import_node_path4.resolve)(consumerRoot, "docs/deloitte-postman-onboarding-email.md"),
    logicApp: (0, import_node_path4.resolve)(consumerRoot, "docs/deloitte-postman-logic-app"),
    config: (0, import_node_path4.resolve)(consumerRoot, DEFAULT_CONFIG_FILE)
  };
  const owned = Object.entries(destinations).filter(([name]) => name !== "config").map(([, path]) => path);
  if (!upgrade) {
    const conflict = (await Promise.all(owned.map(async (path) => ({ path, exists: await exists(path) })))).find((entry) => entry.exists);
    if (conflict) throw new Error(`Starter-kit destination already exists: ${conflict.path}`);
  }
  await (0, import_promises4.mkdir)((0, import_node_path4.resolve)(actionRoot, "dist"), { recursive: true });
  await (0, import_promises4.mkdir)((0, import_node_path4.dirname)(destinations.workflow), { recursive: true });
  await (0, import_promises4.mkdir)((0, import_node_path4.dirname)(destinations.doctor), { recursive: true });
  await (0, import_promises4.mkdir)((0, import_node_path4.dirname)(destinations.runbook), { recursive: true });
  await (0, import_promises4.copyFile)((0, import_node_path4.resolve)(sourceRoot, "action.yml"), (0, import_node_path4.resolve)(actionRoot, "action.yml"));
  await (0, import_promises4.copyFile)((0, import_node_path4.resolve)(sourceRoot, "LICENSE"), (0, import_node_path4.resolve)(actionRoot, "LICENSE"));
  await (0, import_promises4.copyFile)((0, import_node_path4.resolve)(sourceRoot, "README.md"), (0, import_node_path4.resolve)(actionRoot, "README.md"));
  await (0, import_promises4.copyFile)((0, import_node_path4.resolve)(sourceRoot, "dist/index.cjs"), (0, import_node_path4.resolve)(actionRoot, "dist/index.cjs"));
  await (0, import_promises4.copyFile)((0, import_node_path4.resolve)(sourceRoot, "dist/cli.cjs"), (0, import_node_path4.resolve)(actionRoot, "dist/cli.cjs"));
  await (0, import_promises4.copyFile)((0, import_node_path4.resolve)(sourceRoot, "templates/deloitte-postman-workspace-access.yml"), destinations.workflow);
  await (0, import_promises4.copyFile)((0, import_node_path4.resolve)(sourceRoot, "templates/deloitte-postman-pending-reconcile.yml"), destinations.pendingWorkflow);
  await (0, import_promises4.copyFile)((0, import_node_path4.resolve)(sourceRoot, "templates/deloitte-postman-doctor.sh"), destinations.doctor);
  await (0, import_promises4.copyFile)((0, import_node_path4.resolve)(sourceRoot, "docs/SHAROOQ-RUNBOOK.md"), destinations.runbook);
  await (0, import_promises4.copyFile)((0, import_node_path4.resolve)(sourceRoot, "docs/POSTMAN-PREREQUISITES.md"), destinations.prerequisites);
  await (0, import_promises4.copyFile)((0, import_node_path4.resolve)(sourceRoot, "docs/SANDBOX-SMOKE.md"), destinations.sandbox);
  await (0, import_promises4.copyFile)((0, import_node_path4.resolve)(sourceRoot, "docs/NOTIFICATIONS.md"), destinations.notifications);
  await (0, import_promises4.copyFile)((0, import_node_path4.resolve)(sourceRoot, "templates/deloitte-postman-onboarding-email.md"), destinations.emailTemplate);
  await (0, import_promises4.cp)((0, import_node_path4.resolve)(sourceRoot, "templates/logic-app"), destinations.logicApp, { recursive: true, force: true });
  if (!await exists(destinations.config)) {
    await (0, import_promises4.writeFile)(destinations.config, DEFAULT_DELOITTE_CONFIG, { mode: 384 });
  }
  await (0, import_promises4.chmod)((0, import_node_path4.resolve)(actionRoot, "dist/cli.cjs"), 493);
  await (0, import_promises4.chmod)(destinations.doctor, 493);
  return {
    target: consumerRoot,
...
async function runNotificationTest(args) {
  const { values } = (0, import_node_util.parseArgs)({
    args,
    strict: true,
    options: {
  ...
  const delivered = await deliverNotificationEnvelope(envelope, {
    ...
    idempotencyKey: `deloitte-postman:test:${workspaceId}:${(0, import_node_crypto.randomUUID)()}`
  });
...
async function runCli(argv = process.argv.slice(2)) {
  ...
  const identityMapPath = values["identity-map-file"] ?? pathFromConfig(loadedConfig.path, config.scanner?.identityMapFile);
  const configuredScannerRoot = pathFromConfig(loadedConfig.path, config.scanner?.searchRoot);
  const identityMap = await loadIdentityMap(identityMapPath);
  const policy = invalidMemberPolicy(values["invalid-member-policy"]) ?? config.scanner?.invalidMemberPolicy ?? "continue";
  const resolved = await resolveMembersInput(
    values["members-json"],
    values["members-file"],
    values["role-map-json"] ?? (config.roleMap ? JSON.stringify(config.roleMap) : void 0),
    ...
    values["default-workspace-role"] ?? config.defaultWorkspaceRole ?? "Viewer",
    {
      ...
      excludeLogins: stringArrayJson(values["exclude-logins-json"], "--exclude-logins-json") ?? config.scanner?.excludeLogins ?? [],
